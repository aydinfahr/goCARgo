...
#     """
#     Creates a new user with a hashed password and verifies the uniqueness of email and username.
#     """
#     existing_user = db.query(User).filter(
#         (User.email == user_data.email) | (User.username == user_data.username)
#     ).first()
...
def get_user_by_id(db: Session, user_id: int, current_user: User = Depends(get_current_user)):
    """
    Retrieves a user by their unique ID. Access is restricted to the user themselves.
    """
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
...
def get_user_by_email(db: Session, email: str, current_user: User = Depends(get_current_user)):
    """
    Retrieves a user by their email address. Access is restricted to the user themselves.
    """
    user = db.query(User).filter(User.email == email).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
...
def update_user(db: Session, user_id: int, user_update_data: UserUpdate, current_user: User = Depends(get_current_user)):
    """
    Updates user information based on provided data. Access is restricted to the user themselves.
    """
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
...
def delete_user(db: Session, user_id: int, current_user: User = Depends(get_current_user)):
    """
    Deletes a user from the database. Access is restricted to the user themselves.
    """
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
...
def verify_user_email(db: Session, user_id: int, current_user: User = Depends(get_current_user)):
    """
    Verifies a user's email. Access is restricted to the user themselves.
    """
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
...
def verify_user_id(db: Session, user_id: int, current_user: User = Depends(get_current_user)):
    """
    Completes the ID verification process for a user. Access is restricted to the user themselves.
    """
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
...
def change_password(db: Session, user_id: int, new_password: str, current_user: User = Depends(get_current_user)):
    """
    Allows a user to change their password. This action is restricted to the user themselves.
    """
    if current_user.id != user_id:
        raise HTTPException(status_code=403, detail="Not authorized to change password for this user")
